            ...
            <ScrollView className="px-5 flex-1">
                <View className="flex-row">
                    <View className="w-full flex-[.5]">
                        ...
                        { secondHalfCollections.map(item => <ExistingCollection name={item.name} />) }
                    </View>
                    <View className="w-full flex-[.5]">
                        { firstHalfCollections.map(item => <ExistingCollection name={item.name} />) }
                    </View>
                </View>
...
const ExistingCollection = ({ name, containerStyles }) => {
    ...
    return (
        <View className={`bg-[#ffffff] border-[.5px] mb-5 border-[#8A8A8A] rounded-[20px] justify-between p-4 max-w-[169px] w-full max-h-[174px] ${containerStyles}`}>
